...
class Aoe_AmazonCdn_Model_Cdn_Connector
{
    ...
    public function getBucketContents($name, $prefix = null, $only_names = false, $max_keys = null, $marker = null, $delimiter = null)
    {
        $contents = array();
        do {
            $q = array();
    ...
    public function copyObject($bucket_name, $s3_path, $dest_bucket_name, $dest_s3_path)
    {
        $request = array(
            ...
            'resource' => "/$dest_s3_path"
        );
        $headers = array(
            'x-amz-copy-source' => "/$bucket_name/$s3_path"
        );
